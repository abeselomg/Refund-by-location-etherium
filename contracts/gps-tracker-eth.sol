...
contract gpsContract {
    ...
    function time_is_valid(
        ...
        if (fir.hour > end_time.hour || fir.hour < start_time.hour) {
            ...
            if (
                fir.minute > end_time.minute || fir.minute < start_time.minute
            ) {
                return false;
            } else if (
                fir.minute == end_time.minute || fir.minute == start_time.minute
    ...
    function createContract(
        ...
        GpsCordinates memory gps_cor = GpsCordinates(
            latitude_val,
            longtude_val
        );
        DateTime memory start_time = DateTime(
            start_hour,
            start_minute,
            start_second
        );
...
//Pay button
            ...
            DateTime memory start_time = EmployeeContrInfo[msg.sender]
                .startTime;
